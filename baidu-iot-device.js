...
function do_sign(path, expiryTime, body, DEVICE_SECRET) {
    let authStringPrefix = `${path}\n${expiryTime}\n${body}`
    let deviceSecretBytes = Buffer.from(DEVICE_SECRET) // 获取密钥字节数组
...
async function doreq(HOST, PORT, INSTANCE, PRODUCT_KEY, DEVICE_NAME, DEVICE_SECRET) {
    console.debug('doreq is called')
    let expiryTime = String(Math.floor(new Date().getTime() / (1000 * 60)))
    let body = { "resourceType": "MQTT", "permanentConnect": true }
    let path = `/v1/devices/${INSTANCE}/${PRODUCT_KEY}/${DEVICE_NAME}/resources`
    // 获取签名
    let signature = do_sign(path, expiryTime, JSON.stringify(body), DEVICE_SECRET)
...
module.exports = function (RED) {
    ...
    function DeviceBrokerNode(n) {
        ...
        node.setOptions = async function (opts, init) {
            ...
            setIfHasProperty(opts, node, "endpoint", init);
            setIfHasProperty(opts, node, "endpointport", init);
            setIfHasProperty(opts, node, "instanceid", init);
            setIfHasProperty(opts, node, "productkey", init);
            setIfHasProperty(opts, node, "devicename", init);
            setIfHasProperty(opts, node, "devicesecert", init);
            setIfHasProperty(opts, node, "effectivetime", init);
            ...
            node.options.hostname = node.broker;
    ...
    RED.nodes.registerType("device-broker", DeviceBrokerNode, {
        credentials: {
            user: { type: "text" },
    ...
    //#region  "MQTTIn node"
    function DeviceCommandInvokeNode(n) {
        RED.nodes.createNode(this, n);
    ...
    RED.nodes.registerType("command invoke", DeviceCommandInvokeNode);
    //#endregion  "MQTTIn node"

    //#region "MQTTOut node"
    function DevicePropertyPostNode(n) {
        RED.nodes.createNode(this, n);
        const node = this;
        node.topic = n.topic;
        node.qos = n.qos || null;
    ...
    RED.nodes.registerType("property post", DevicePropertyPostNode);

    //#region "MQTTOut node"
    function DeviceEventPostNode(n) {
        RED.nodes.createNode(this, n);
        const node = this;
        node.topic = n.topic;
        node.qos = n.qos || null;
    ...
    RED.nodes.registerType("event post", DeviceEventPostNode);
